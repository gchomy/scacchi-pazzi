...
namespace Sakk_Alkalmazás_2._0
{
    public partial class InGameForm : Form
    {
        ...
        #endregion
        #region Log
        List<LogMossa> start = new List<LogMossa>();
        LogMossa current = new LogMossa(0, 0, 0, 0);
        string mossa;
        int pos = 0;
        #endregion
        ...
        public InGameForm(bool SingleGame, bool isHost, bool ViewLog, string ip = null)
        {
            InitializeComponent();

            singleGame = SingleGame;
            viewLog = ViewLog;
            //if the user wants to see the log
            List<int> lista = new List<int> { 6, 4, 4, 4, 1, 4, 3, 4, 7, 3, 3, 7, 0, 1, 2, 2, 7, 5, 4, 2, 0, 6, 2, 5, 3 , 7, 1 , 5 , 0};
            if (viewLog)
            {
            ...
            if (!SingleGame)
            {
                ...
                else
                {
                    ...
                    {
                        MessageBox.Show(ex.Message, "Lanchess");
                        Close();
                    }
                }
            }
        ...
        //save moves in an logMossa list
        public void saveMosse(List<int> lista)
        {
            int tmp = -1;
            for (int i = 0; i < lista.Count; i++)
            {
                tmp += 1;
                if (tmp == 0) { current.preI = lista[i]; }
                else if (tmp == 1) { current.preJ = lista[i]; }
                else if (tmp == 2) { current.postI = lista[i]; }
                else if (tmp == 3) { current.postJ = lista[i]; }
                else if (tmp == 4)
                {
                    i -= 1;
                    tmp = -1;
                    int dc = current.preI;
                    int dp = current.preJ;
                    int dm = current.postI;
                    int dl = current.postJ;
                    start.Add(new LogMossa(dc, dp, dm, dl));
                }
            }
            current = start[0];
        }

        public int lifecolumn(int i)
        {
            return 8 - i;
        }

        public char deathrow(int j)
        {
            List<Char> charchess = new List<Char> { 'A', 'B', 'C', 'D', 'E', 'F', 'G', 'H' };
            return charchess[j];
        }
        ...
        //nuova funzione bellissima per fare le mosse come un pazzo
        void DoMoves()
        {
            AfterClickOnTable(current.preI, current.preJ);
            AfterClickOnTable(current.postI, current.postJ);
        }
        ...
        public void AfterClickOnTable(int i, int j)
        {
            switch (tableClass.PossibleMoves[i, j])
            {
                ...
                case 2:
                    ...
                    mossa = deathrow(BeforeMove_I) + "-" + lifecolumn(BeforeMove_J) + " -> " + deathrow(i) + "-" + lifecolumn(j) + " ";
                    RTBMosse.Text = RTBMosse.Text + tmp + ": " + mossa + "\n\n" ;
                    Moves = 0;
                    SuccesfulMove(i,j);
                    break;
        ...
        public void CheckMateChecker(int a, int b)
        {
            if (Moves == 0)
            {
                ...
                MessageBox.Show("HAI VINTO!", "Lanchess");
            }
        }
        //socket things
        ...
        private void ReceiveMove()
        {
            ...
            if (buffer[5] == 0)
            {
                if (WhiteTurn)
                {
                    MessageBox.Show("Hai perso!", "Lanchess");
                }
                else
                {
                    MessageBox.Show("Hai perso!", "Lanchess");
        ...
        private void nextMove_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Questa è l'ultima mossa salvata", "LanChess");
            }
        }
